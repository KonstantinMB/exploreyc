...
//
// Ported unchanged from the retiring deck.gl map (components/WorldMap/batchOrder.ts)
// when the two globes merged. Behaviour is identical by design — the batch
// filter on /world orders batches exactly as /map did. Dependency-free and
// pure, so it is a straight copy rather than a re-derivation.
